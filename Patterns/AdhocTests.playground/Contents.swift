...
class Test {
    lazy private var serialQueue:dispatch_queue_t = {
        print("serial CREATE")
        return dispatch_queue_create("com.davidbjames.queues.singleton.worker.serial", DISPATCH_QUEUE_SERIAL)
        }()
    lazy private var concurrentQueue:dispatch_queue_t = {
        print("concurrent CREATE")
        return dispatch_queue_create("com.davidbjames.queues.singleton.worker.concurrent", DISPATCH_QUEUE_CONCURRENT)
        }()
